...
export interface TikTokInput {
   ...
   * override. When present (and > 0) it REPLACES the category/manual rate
   * entirely — the standard, category and promotional rates are never
   * stacked/summed.
   */
  promotionalRate?: number | null;
  /**
   * Optional, user-entered affiliate/creator commission rate (fraction,
   * e.g. 0.1 = 10%). Never assumed — omitted entirely (not defaulted to
   * 0-as-"typical") when not supplied. Must fall within TikTok's currently
   * documented 1%-80% range when supplied; applied to its own basis (product
   * price minus seller and platform discount, excluding shipping) —
   * separate from, and never combined with, the platform commission basis.
   */
  affiliateCommissionRate?: number | null;
  /**
   * Lump-sum actual costs for TikTok Shop fulfilment/FBT, ads, storage and
   * returns — entered as-is by the seller. TikTok does not publish a fee
   * schedule for these that this build's verified dataset covers, so no
   * schedule is invented or generalised; this is a direct actual-cost entry,
   * the same pattern the app already uses for "Actual Shipping Cost".
   */
...
export function calculateTikTok(input: TikTokInput): CalculationResult {
  ...
  // (never below £0). Unlike the platform basis, this one EXCLUDES
  // customer-paid shipping and SUBTRACTS the platform discount rather than
  // ignoring it. Same worked example: 100 - 10 - 10 = £80.
  const affiliateBasisRaw = itemSubtotal.minus(input.sellerDiscount).minus(input.platformDiscount);
  const affiliateBasis = affiliateBasisRaw.lt(0) ? ZERO : affiliateBasisRaw;
  ...
  if (commissionRate !== null) {
    ...
    feeLines.push(
      makeFeeLine({
        ...
        notes:
          "TikTok Shop's published commission rate is inclusive of applicable VAT — no separate 20% UK VAT is added on top of it.",
      })
    );
  } else {
    exclusions.push(
  ...
  if (isValidAffiliateRate(input.affiliateCommissionRate)) {
    ...
    feeLines.push(
      makeFeeLine({
        ...
        formula: `${formatPercent(input.affiliateCommissionRate)} on £${toRawNumber(affiliateBasis).toFixed(2)} (£${toRawNumber(itemSubtotal).toFixed(2)} product price - £${input.sellerDiscount.toFixed(2)} seller discount - £${input.platformDiscount.toFixed(2)} platform discount, floored at £0 — excludes customer-paid shipping; a separate basis from the platform commission)`,
        vatUnconfirmed: true,
        notes: 'User-entered affiliate/creator program rate — never assumed when not supplied. VAT treatment is not modelled; confirm with your accountant.',
      })
    );
  }
  ...
  if (otherActualCosts.gt(0)) {
    feeLines.push(
      makeFeeLine({
        ...
        vatUnconfirmed: true,
        notes:
          "Entered as an actual cost you already know, not derived from a published fee schedule — TikTok's FBT/ads/storage/return fee schedules are variable and were not part of the verified dataset for this build.",
      })
    );
  }
